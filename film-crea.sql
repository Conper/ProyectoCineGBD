...
CREATE SEQUENCE directores_seq;
...
CREATE TABLE directores (
    codigo NUMBER DEFAULT directores_seq.NEXTVAL CONSTRAINT directores_pk PRIMARY KEY,
...
CREATE TABLE direccion_peli (
    ...
    codigo_direc NUMBER REFERENCES directores(codigo) ON DELETE CASCADE,
    CONSTRAINT direccion_peli_pk PRIMARY KEY (id_peli, codigo_direc)
);

CREATE TABLE actores (
    codigo NUMBER DEFAULT actores_seq.NEXTVAL CONSTRAINT actores_pk PRIMARY KEY,
    nombre VARCHAR2(32) NOT NULL,
    apellido1 VARCHAR2(32) NOT NULL,
    apellido2 VARCHAR2(32),
    fecha_nac DATE
...
CREATE TABLE protas_peli (
    id_peli NUMBER REFERENCES peliculas(id) ON DELETE CASCADE,
    codigo_actor NUMBER REFERENCES actores(codigo) ON DELETE CASCADE,
